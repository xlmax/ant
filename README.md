...
Введите `/help`, чтобы увидеть локальные команды REPL: `/new`, `/session`, `/clear`, `/context`, `/model`, `/think`, `/reasoning`, `/exit` и справку по отдельной команде. `/context` локально оценивает использование контекстного окна, показывает разбивку и самые тяжёлые результаты инструментов; команда не обращается к API и не изменяет сессию. Вывод разделён цветными блоками; для их отключения задайте `ui.color: false` в настройках.
