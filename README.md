...
Настройки без секретов загружаются по слоям: `~/.ant/settings.json`, затем `.ant/settings.json` в рабочей директории. Проектный файл перекрывает глобальный, в том числе `ui.showReasoning`.
