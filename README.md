...
Введите `/help`, чтобы увидеть локальные команды REPL: `/new`, `/session`, `/clear`, `/context`, `/compact`, `/model`, `/think`, `/reasoning`, `/exit` и справку по отдельной команде. `/context` локально оценивает использование контекстного окна, показывает разбивку и самые тяжёлые результаты инструментов; команда не обращается к API и не изменяет сессию. `/compact` обращается к активной модели, сворачивает старую часть истории в структурированное резюме и сохраняет последние два пользовательских хода дословно. Исходные события остаются в JSONL: новое compaction-событие меняет только контекст, отправляемый модели. Вывод разделён цветными блоками; для их отключения задайте `ui.color: false` в настройках.
