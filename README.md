...
Каждый запуск создаёт JSONL-журнал в `.ant/sessions/<session-id>.jsonl`. В нём сохраняются задачи, решения модели и результаты инструментов; служебные события (попытки запроса, повторы и статистика токенов) в журнал не пишутся. Директория игнорируется Git.
