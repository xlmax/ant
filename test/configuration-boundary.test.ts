...
test("filesystem configuration service is section-agnostic", async () => {
  const service = await readFile(source("config/configuration-service.ts"), "utf8");
...
test("composition root registers configuration sections explicitly", async () => {
  const main = await readFile(source("main.ts"), "utf8");
  const applicationContract = await readFile(source("app/configuration.ts"), "utf8");
...
test("DeepSeek module owns model defaults, migration and secret policy", async () => {
  const section = await readFile(source("models/deepseek-configuration-section.ts"), "utf8");
