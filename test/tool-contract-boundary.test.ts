...
test("application owns the public tool and registry contracts", async () => {
  const applicationContract = await readFile(source("app/tools.ts"), "utf8");
  const environmentAdapter = await readFile(source("tools/tool-environment.ts"), "utf8");
  ...
  assert.match(environmentAdapter, /\.\.\/app\/tools\.js/u);
